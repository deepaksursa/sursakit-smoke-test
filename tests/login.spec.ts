  ...
  test('TC_001: Valid Login (Complete POM)', async ({ page }) => {
    ...
    await test.step('Analyze Results with Page Objects', async () => {
      ...
      if (loginPage.isOnLoginPage()) {
        ...
        if (errors.some(err => err?.toLowerCase().includes('captcha') || err?.toLowerCase().includes('service'))) {
          console.log('   🎯 CAPTCHA SERVICE ERROR DETECTED!');
          console.log('   • Backend is expecting CAPTCHA validation');
          console.log('   • Frontend has Turnstile disabled but backend still requires it');
          console.log('   • Check your backend logs for "CAPTCHA service unavailable"');
          console.log('');
          console.log('🔧 SOLUTIONS:');
          console.log('   1. Disable CAPTCHA plugin in your backend auth config');
          ...
          console.log('💡 CHECK: Review screenshots and update .env credentials');
        }
